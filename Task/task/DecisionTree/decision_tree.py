...
class DecisionTree:
    ...
    def _validator(self, user_data, product_data, validation):
        ...
        if "allowed_values" in validation:
            for criteria in validation["allowed_values"]:
                ...
                if validation["allowed_values"][criteria] == False:
                    ...
                    criteria == "min"
                    and value < validation["allowed_values"][criteria]
                    and not is_cat
                ):
                    # print('Min')
                    return False
                    ...
                    criteria == "list"
                ):  # and value not in validation['allowed_values'][criteria]:
        ...
        if "excluded_values" in validation:
            for criteria in validation["excluded_values"]:
                ...
                if (
                    criteria == "min"
                    and value > validation["excluded_values"][criteria]
                    and not is_cat
                ):
                    return False
                ...
                if (
                    criteria == "list"
                    and value in validation["excluded_values"][criteria]
                ):
                    return False
        return True
    ...
    def validate_user_for_product(
        self, user_data, product_data, decisions, category, product
    ):
    ...
    def validate_persona_for_user(
        self, persona_data, user_data, decisions, category, persona
    ):
